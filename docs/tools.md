...
- **SOAP 中介起單**（原生設計的表單）：支援單站自由流程 + 基本欄位型別（文字、自動編號、可空欄位、
  ...
  固定流程逐站推進尚未支援。
- **網頁起單**（本體是客製 plugin、中介欄位填不到內容的表單，如採購單）：以 Playwright 驅動網頁完整
  填單送出（主旨/供應商/明細等）。較慢、較依賴頁面結構；簽核者由表單自身流程決定。
...
Playwright 填網頁表單，仍沿用同一個 `apply_form` 入口。
...
以網頁取得清單（對使用者透明）。**認證走 web session（帳號/密碼登入），不需 SOAP token**，因此沒有
PublicAPI 的站台也能用；部署時需先 `uv run playwright install chromium`。
